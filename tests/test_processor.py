...
def test_dirs():
    ...
    downloads = os.path.join(base, "test_downloads")
    destination = os.path.join(base, "test_destination")
    os.makedirs(downloads)
    os.makedirs(destination)
    yield downloads, destination
    shutil.rmtree(base)


@pytest.fixture
def mock_config(test_dirs):
    downloads, destination = test_dirs
    return {
        "directories_to_watch": [downloads],
...
def test_fallback_classification(mock_config):
    ...
    cat, conf = processor.classify_file("fake_path.pdf", "meu_arq.pdf")
    assert cat == "Documentos_Teste"
    assert conf is None

    cat, conf = processor.classify_file("fake_path.jpg", "foto_ferias.jpg")
    assert cat == "Imagens_Teste"

    cat, conf = processor.classify_file("fake_path.xyz", "desconhecido.xyz")
    assert cat == "Outros"


def test_process_file_integration(mock_config, test_dirs):
    downloads, destination = test_dirs
    processor = FileProcessor(mock_config)
    test_file_path = os.path.join(downloads, "nota_urgente.txt")
    ...
    expected_dest_dir = os.path.join(destination, "Textos_Teste")
    assert os.path.exists(expected_dest_dir)
    assert os.path.exists(os.path.join(expected_dest_dir, "nota_urgente.txt"))


def test_temp_file_ignore(mock_config, test_dirs):
    downloads, destination = test_dirs
    processor = FileProcessor(mock_config)
    temp_file = os.path.join(downloads, "fatura.pdf.crdownload")
...
def test_safety_prevent_overwrite(mock_config, test_dirs):
    downloads, destination = test_dirs
    processor = FileProcessor(mock_config)

    dest_dir = os.path.join(destination, "Textos_Teste")
    os.makedirs(dest_dir, exist_ok=True)
    existing_file = os.path.join(dest_dir, "importante.txt")
    with open(existing_file, "w") as f:
        f.write("DADOS ANTIGOS")

    new_file = os.path.join(downloads, "importante.txt")
...
def test_ai_classification_mock(test_dirs):
    downloads, destination = test_dirs
    config_com_ia = {
        "destination_base_folder": destination,
        "ai_classification": {
    ...
    test_file_path = os.path.join(downloads, "doc_ia.txt")
    with open(test_file_path, "w") as f:
        f.write("Testando a rede neural!")

    processor.process_file(test_file_path)

    expected_dest_dir = os.path.join(destination, "Seguranca_IA")
    assert os.path.exists(expected_dest_dir)
    assert os.path.exists(os.path.join(expected_dest_dir, "doc_ia.txt"))


def test_process_existing_files(mock_config, test_dirs):
    downloads, destination = test_dirs
    processor = FileProcessor(mock_config)

    regular_file = os.path.join(downloads, "documento.pdf")
    with open(regular_file, "w") as f:
        f.write("Conteudo pdf fake")

    hidden_file = os.path.join(downloads, ".gitkeep")
    with open(hidden_file, "w") as f:
        f.write("")

    sub_dir = os.path.join(downloads, "subpasta")
    os.makedirs(sub_dir, exist_ok=True)

    other_base = os.path.dirname(downloads)
    os.makedirs(os.path.join(other_base, "test_other_dir"), exist_ok=True)
    outside_file = os.path.join(other_base, "test_other_dir", "nao_mexer.txt")
    ...
    assert os.path.exists(
        os.path.join(destination, "Documentos_Teste", "documento.pdf")
    ), "Ficheiro devia estar no destino"
